...
class ImageClassifier(BaseClassifier):
    ...
    def forward(self,
                ...
                inputs_raw: Optional[torch.Tensor] = None,
                inputs2_raw: Optional[torch.Tensor] = None,
        ...
        """
        if mode == 'tensor':
            feats = self.extract_feat(inputs, inputs2, mode=mode, inputs_raw=inputs_raw, inputs2_raw=inputs2_raw, epoch=epoch, iter_idx=iter_idx)
            return self.head(feats[-1]) if self.with_head else feats[-1]
        elif mode == 'loss':
            return self.loss(inputs, inputs2, data_samples, inputs_raw=inputs_raw, inputs2_raw=inputs2_raw, epoch=epoch, iter_idx=iter_idx, mode=mode)
        elif mode == 'predict':
            return self.predict(inputs, inputs2, data_samples, inputs_raw=inputs_raw, inputs2_raw=inputs2_raw, epoch=epoch, iter_idx=iter_idx, mode=mode)
        else:
            raise RuntimeError(f'Invalid mode "{mode}".')

    def extract_feat(self, inputs, inputs2, inputs_raw, inputs2_raw, epoch=None, iter_idx=None, stage='neck', mode='loss'):
        ...
        x = self.backbone(inputs, inputs2, mode, inputs_raw=inputs_raw, inputs2_raw=inputs2_raw, epoch=epoch, iter_idx=iter_idx)
    ...
    def predict(self,
        ...
        Args:
            ...
            **kwargs: Other keyword arguments accepted by the ``predict``
                method of :attr:`head`.
        """
        feats = self.extract_feat(inputs, inputs2, mode=mode, inputs_raw=inputs_raw, inputs2_raw=inputs2_raw)
