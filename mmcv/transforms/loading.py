...
class LoadImageFromFileMine(BaseTransform):
    ...
    def get_random_image_path(self, base_path):
        ...
        # 筛选出所有图片文件
        image_files = [f for f in all_files if f.endswith(('.jpg','.JPG', '.jpeg', '.png'))]
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        # TODO：作为对比，在这里也保存一下两张图片，用最傻的cv2看看是不是一样的图
        # cvimg = cv.imread(filename)
        # cv.imwrite('cv2read.jpg', cvimg)
        filename2 = self.get_random_image_path(filename)
        try:
            ...
            # TODO：在这里保存一下两张图片，看一下是不是已经图片和原图不一样了
        ...
        results['img_shape2'] = img2.shape[:2]
        results['ori_shape'] = img.shape[:2]
        results['ori_shape2'] = img2.shape[:2]
        return results
